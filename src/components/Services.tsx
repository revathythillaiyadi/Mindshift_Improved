...
export default function Services() {
  return (
    <section id="services" className="py-20 px-6 bg-gradient-to-b from-amber-50/40 via-warm-white to-amber-100/20 relative overflow-hidden">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
          {services.map((service, index) => {
            ...
            return (
              <div
                key={service.title}
                className="bg-white/80 backdrop-blur-sm p-8 rounded-[2rem] shadow-lg border-2 border-amber-100/50 hover:shadow-xl transition-all hover:-translate-y-2 group relative overflow-hidden animate-fade-in"
                style={{ animationDelay: `${index * 0.1}s` }}
              >

                <div className="w-16 h-16 bg-gradient-to-br from-amber-600 to-amber-400 rounded-[1.5rem] flex items-center justify-center mb-6 group-hover:scale-110 group-hover:rotate-3 transition-all shadow-md relative">
                ...
                <div className="absolute bottom-0 left-0 right-0 h-1 bg-gradient-to-r from-amber-400/20 via-amber-500/20 to-amber-400/20 rounded-b-[2rem]">
                  <div className="h-full w-0 group-hover:w-full bg-gradient-to-r from-amber-600 to-amber-400 transition-all duration-500 ease-out" />
                </div>
              </div>
            );
          })}
